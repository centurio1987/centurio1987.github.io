 ...
 *
 * **이 표는 서체를 모른다 — 코드포인트 구간으로만 폭을 매긴다.** 그래서 서체나 굵기가
 * 바뀌면 실폭은 바뀌는데 이 계산은 그대로다. KAN-080 `S17` 이 그 자리를 다시 봤고
 * 결론은 「지금은 안 헐거워진다」이다:
 *   · 서체는 안 바뀌었다 — `S2` 가 **「교체하지 않는다」**로 닫혔다(굵기 단만 굽는다).
 *   · 굵기는 얇아지는 쪽으로만 움직였다 — `PosterHero` 제목이 800 → 700 이라 실폭이
 *     **좁아진다.** 넘침 방어선이 느슨해지는 방향이 아니다.
 * 서체를 실제로 갈아 끼우거나 굵기를 **올리는** 변경이 오면 그때는 이 표를 다시 재야 하고,
 * **`bun run viz:verify` 가 유일한 방어선**이다(SVG `<text>` 는 리플로도 클리핑도 안 해서
 * 넘친 글자가 에러 없이 잘린다).
 */
