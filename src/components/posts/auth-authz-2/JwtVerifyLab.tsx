...
export default function JwtVerifyLab() {
  ...
  const optBtn = (on: boolean): React.CSSProperties => ({
    padding: "6px 10px",
    border: on ? `2px solid ${TEAL}` : "1px solid #c9c1b1",
    borderRadius: 7,
  ...
  const label: React.CSSProperties = { fontSize: 12, fontWeight: 600, color: "#6b6357" };
  ...
  return (
    ...
    >
      ...
      <button
        ...
        style={{
          ...optBtn(weak),
          border: weak ? `2px solid ${RED}` : "1px solid #c9c1b1",
          background: weak ? "#f4e3e0" : "#fffdf8",
          marginBottom: 14,
        }}
      >
        ...
        {result.steps.map((s) => (
          <div
            ...
            style={{
              ...
              borderRadius: 8,
              background: s.ok ? "#fffdf8" : "#f7ebe9",
              lineHeight: 1.5,
            }}
          >
            <strong style={{ color: s.ok ? TEAL : RED }}>{s.ok ? "✔" : "✘"}{s.n}</strong>
            <strong style={{ fontSize: 13, color: "#302d28" }}>{s.name}</strong>
            <span style={{ fontSize: 13, color: "#4a463f" }}>{s.detail}</span>
          </div>
        ))}
      </div>
