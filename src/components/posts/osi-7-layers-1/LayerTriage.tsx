...
const symptoms: Symptom[] = [
  {
    id: "link",
    label: "링크 LED가 꺼져 있고 interface가 DOWN입니다",
    layer: "L1 Physical",
    layerNo: 1,
    why: "신호 자체가 매체에 실리지 못하는 상태입니다. 상위 계층 설정은 아직 볼 차례가 아닙니다.",
    ...
    tip: "간헐적이면 error counter(`ip -s link`)의 CRC·carrier 증가량부터 봅니다. 케이블만 바꿔도 끝나는 일이 의외로 많습니다.",
  },
  {
    id: "neigh",
    label: "gateway가 ping되지 않고 neighbor가 INCOMPLETE입니다",
    layer: "L2 Data Link",
    layerNo: 2,
    why: "같은 링크의 다음 장치(보통 gateway)의 MAC을 못 알아내는 상태입니다. ARP/Neighbor Discovery가 응답을 못 받고 있습니다.",
    ...
    tip: "이 단계에서 DNS 서버를 바꾸는 것은 원인과 멉니다. gateway가 같은 링크(on-link)로 잡히는지, 같은 VLAN인지부터 확인합니다.",
  },
  {
    id: "route",
    label: "gateway는 되는데 외부 IP(예: 1.1.1.1)로 못 나갑니다",
    layer: "L3 Network",
    layerNo: 3,
    why: "링크는 살아 있으나 다른 네트워크로 가는 경로·주소가 어긋난 상태입니다. default route나 NAT가 의심됩니다.",
    ...
    tip: "ping 실패만으로 목적지가 죽었다고 단정하지 않습니다. ICMP가 차단됐을 수 있으니 실제 service traffic도 함께 봅니다.",
  },
  {
    id: "port",
    label: "IP는 ping되는데 서비스 포트에 연결되지 않습니다",
    layer: "L4 Transport",
    layerNo: 4,
    why: "host까지는 도달하지만 원하는 process의 port까지 transport 연결이 안 되는 상태입니다. firewall이나 listen 여부 문제입니다.",
    ...
    tip: "refused와 timeout은 전혀 다른 단서입니다. refused는 '명시적 거절을 받았다', timeout은 '어딘가에서 조용히 삼켜졌다' — 다음에 볼 곳이 갈립니다.",
  },
  {
    id: "tls",
    label: "연결은 되는데 인증서/TLS 오류가 납니다",
    layer: "L6 Presentation",
    layerNo: 6,
    why: "패킷은 이미 잘 오가지만 양쪽이 bytes를 같은 의미로 해석·보호하지 못하는 상태입니다. 인증서·SNI·시계 문제입니다.",
    ...
    tip: "로컬 시계가 틀어지면 멀쩡한 인증서도 만료/미발효로 보입니다. `date`부터 확인하는 습관이 시간을 아낍니다.",
  },
  {
    id: "http",
    label: "TLS까지 멀쩡한데 503/404/타임아웃이 뜹니다",
    layer: "L7 Application",
    layerNo: 7,
    why: "하위 전달은 모두 성공했고 application 의미에서 실패한 상태입니다. '네트워크 문제'가 아니라 서버·라우팅·upstream 문제입니다.",
    command: "curl -v https://example.com/ -o /dev/null",
    healthy: "HTTP/2 200, 기대한 본문",
    broken: "404(라우팅/경로), 503(upstream 다운/과부하), 응답 지연(백엔드 timeout) — application log를 봅니다",
    tip: "여기까지 왔으면 패킷은 죄가 없습니다. '인터넷이 안 돼요' 대신 '어느 책임의 실패'인지 말할 수 있어야 합니다.",
  },
];
