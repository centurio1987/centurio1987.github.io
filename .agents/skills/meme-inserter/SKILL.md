...
```bash
python3 .Codex/skills/meme-inserter/scripts/find_markers.py "<파일 경로>"
```
...
```bash
bun run .Codex/skills/meme-inserter/scripts/fetch-meme.ts "<정제 쿼리>" "public/images/<slug>/memes/<n>"
```
...
### 6. 저장 + 요약 보고
frontmatter·JSX import·```viz```·잔존 레거시 마커(`[[[ ]]]`·```figure```·`(( ))`) 등 다른 영역을 건드리지 않았는지 확인하고 저장. 짧게 보고:
...
- **`<<meme:>>`만 건드린다.** ```viz```·JSX(react-sim)·frontmatter, 그리고 잔존 레거시 마커(`[[[ ]]]`·```figure```·`(( ))`)는 절대 손대지 않는다.
...
- **실재하는 밈만.** AI 밈 생성 금지 — 여기선 인터넷에서 검색해 실재하는 짤·움짤을 가져온다.
...
- **draft → post (post-finalize):** post-finalize가 후처리(태그·시리즈) 단계에서 문서에 `<<meme:>>`가 남아 있으면 이 스킬을 위탁 호출해 함께 처리한다(발행 직전 한 곳에서 밈까지 완성).
...
## 안 하는 것
- AI 밈/이미지 생성 → 하지 않는다(레포는 AI 이미지 생성을 쓰지 않음).
- 개념도·비교표·다이어그램 등 구조형 시각물 → ```viz```(make-image, viz 엔진) 담당.
