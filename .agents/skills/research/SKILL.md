...
description: >
  ...
  (불변 원본)로 저장하고, ② `~/blog-research/AGENTS.md`(LLM Wiki 스키마)의 Ingest 연산대로
...
### 3. wiki ingest (`~/blog-research/AGENTS.md` Ingest 연산)

`~/blog-research/AGENTS.md`를 읽고 그 스키마대로 통합한다(이 스킬이 규약을 재정의하지 않는다):
...
- `~/blog-research/AGENTS.md` — LLM Wiki 스키마(Ingest/Query/Lint 연산, 페이지 규약). **ingest는 이 문서를 따른다.**
- `scripts/git-commit-push.sh` — 공유 git 가드(git-shipper가 호출).

## 주의

- **raws 불변**: 수정·삭제 금지. NNN 시퀀스는 항상 기존 최대+1로 자동 산정(충돌 주의).
- ingest 규약의 출처는 `~/blog-research/AGENTS.md`다 — 이 스킬은 그걸 *실행*할 뿐, 규약을 바꾸지 않는다.
