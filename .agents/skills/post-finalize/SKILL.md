...
description: >
  ...
  본문 내 `[[[...]]]` 단서(**개념·은유 일러스트 전용**)를 OpenAI 이미지 API로 생성한 webp로 치환하고,
  본문에서 핵심 키워드를 추출해 frontmatter `tags`에 넣고,
  frontmatter `series` 가 있으면 같은 시리즈 다른 글의 링크 목록을 본문 하단에 삽입한다.
  비교표·단계 다이어그램 같은 **구조형 정보 이미지(```figure``` 블록)는 이 스킬이 아니라 `make-image`가 담당**한다.
...
### 3. `[[[...]]]` 이미지 자리표시자 처리 (개념·은유 일러스트 전용)

> **경계**: 이 스킬은 `[[[...]]]`(개념·은유 자유 회화)만 처리한다. 구조형 정보 이미지(비교표·단계 다이어그램·
> 핵심 포인트 카드·인용 박스)는 집필 단계가 ```` ```figure ```` 명세 블록으로 남겨 **`make-image`가 이미 생성·치환**했다.
> 여기서 ```figure``` 블록을 만나면 손대지 말고(아직 남아 있으면 make-image 누락이므로) 보고만 한다. 정규식이 비충돌이라 서로 침범하지 않는다.
...
### 3.5. `<<meme: ...>>` 밈 자리표시자 처리 (위탁)

`[[[...]]]` 처리 직후, 문서에 `<<meme: ...>>` 마커가 남아 있으면 **`meme-inserter` 스킬에 위탁**한다
(실재 인터넷 밈·움짤 검색·다운로드·치환). 정규식이 비충돌이라 `[[[ ]]]`·`(( ))`·```figure```와 침범하지 않는다.
...
- **MDX 안전.** `.mdx` 글은 상단 `import` 문과 `<Component .../>`(React 시뮬레이션) 블록을 frontmatter처럼 보존하고 절대 깨지 말 것. `[[[...]]]` 자리표시자는 산문 영역에만 두며 JSX 블록 내부에는 두지 않는다(치환 결과는 마크다운 `![](...)` 이므로 산문에선 안전). 태그 추출 시 import 경로·컴포넌트명 같은 코드 토큰은 키워드로 넣지 않는다.
