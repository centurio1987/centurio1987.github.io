...
- **게이트는 하드월이 아니라 래칫이다** — 커밋된 기준선 대비 **늘면 실패**한다. 부채가 네
  ...
  다섯째를 열 때마다 다시 비0 이 됐다. 아직 안 연 갈래가 **하나** 남아 있다(KAN-077 상수 승격).
  그것이 닫히기 전에 올리면 그 카드가 **CI 를 빨갛게 만들면서 착수**하게 되고 그동안 배포가 멈춘다.
...
- **게이트의 「0」은 인식 범위 안에서의 0 이다 (KAN-073).** KAN-072 가 래칫을 0 으로 만들자마자
  ...
  **똑같은 코드로** 판정되므로 사유가 겹친다). 아직 안 여는 것 하나는 진단 정본 §6 의 13항이다.
