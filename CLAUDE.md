...
- **게이트는 자기를 검사한다.** 고장을 일부러 주입해 잡히는지 보는 자가검사를 함께 둔다
  ...
  **승격해도 안 없어진다** — 게이트가 조용히 죽으면 판정이 0건이 되고, 하드월은 그것을
  "무는 것이 없다"로, 래칫은 "줄었다"로 읽어 **둘 다 통과시킨다.** 판정 규칙 자체가 되돌아가는
  것까지 보려면 자가검사가 그 규칙도 봐야 한다(`verify-tokens` 의 판정 방식 검사 넷).
...
- **게이트는 하드월이다 — 위반·드리프트가 하나라도 있으면 실패한다 (KAN-079).**
  **기준선과 비교하지 않는다.** 기준선 파일에 무슨 수가 적혀 있어도 판정이 안 흔들리고,
  기준선이 아예 없어도 문다(래칫 시절에는 「기준선 없으면 안 문다」였는데, 그러면 그 파일을
  지우는 것이 곧 게이트를 끄는 것이었다). **`--update-baseline` 은 위반을 굳히는 문이 아니다** —
  무는 판정이 있으면 아무것도 안 쓰고 비0 으로 끝나며, `writeBaseline` 자체가 무는 판정 칸이
  실린 기준선을 거부한다(파일 형식의 불변식이라 배선이 지워져도 남는다).
  **기준선 파일이 하는 일은 셋으로 줄었다** — ① `selfTestFaults`·`selfTestGuards` 래칫
  (게이트 자신의 크기. **줄면 실패** — 승격 뒤에도 래칫으로 남는 자리는 여기뿐이다)
  ② `srcFiles` 알림 ③ 정보 집계와 인식 범위 잣대(`srcVerdicts`·`srcLabels`)의 diff.
  **처음에는 래칫이었다** — 부채가 네 자릿수인 상태에서 "0건이면 통과"로 올리면 게이트가
  `build` 잡 안에서 `astro build` 앞에 돌고 `deploy` 잡이 `needs: build` 로 물려 있어
  **사이트 배포까지 멈춘다.** 승격은 네 번 미뤄졌다(KAN-072 · KAN-074 · KAN-075 · KAN-076 이
  ...
  **KAN-077 로 그 조건이 처음 충족됐고 KAN-079 가 승격했다.** 승격을 별건으로 뺀 이유는
  하드월이 **CI 를 빨갛게 만들면서 착수**할 위험을 지기 때문이다 — 인식층을 넓힌 커밋과 섞으면
  되돌릴 단위가 뭉개진다. 진단 정본 §6 에 남은 13항은 **지금 `src/**` 에 0건**이라 열 대상이
  아니다(「0건이라 괜찮다」가 아니라 「0건인 동안 괜찮다」는 기록이다).
...
- **하드월을 피하는 길이 하나 열려 있다 — 판정 불가로의 재분류 (KAN-079).** 통과하는 길 셋 중
  예외 쪽은 막혀 있지만(위치 없는 예외는 안 받고 예외 표 자체를 자가검사한다) 판정 불가에는
  그 문이 없다. **실패로 물 수는 없다** — 판정 불가가 느는 것은 인식층 확장에서도, `200px` 을
  `100%` 로 바꾸는 평범한 작업에서도 정상 신호다. 그래서 **무는 대신 가른다**: 기준선이
  인식 범위 잣대 셋(`srcFiles`·`srcVerdicts`·`srcLabels`)을 함께 들고, 판정 불가가 늘었는데
  셋이 하나도 안 움직였으면 「새 히트가 아니라 **있던 판정이 옮겨간 것**」이라고 `⚠` 표식으로
  말한다. 남은 구멍(「어느 규칙에서 나왔는가」 — 사유가 산문이라 판정 키로 못 쓴다)과 닫는 법은
  진단 정본 §6-16 에 있다.
- **게이트의 「0」은 인식 범위 안에서의 0 이다 (KAN-073).** KAN-072 가 래칫을 0 으로 만들자마자
  ...
  **인식층을 넓히면 그 커밋에서 기준선도 함께 갱신한다** — 나누면 그 사이 푸시에서 배포가 멈춘다.
  단, 승격 뒤로 **갱신은 무는 판정이 0 일 때만 된다** — 새로 연 갈래가 위반을 물고 오면
  기준선에 굳히는 것이 아니라 그 자리를 먼저 고친다.
...
- **인식층을 넓힐 때 옛 히트가 안 움직였음은 `bun run tokens:invariant` 가 증명한다 (KAN-075).**
  ...
  게이트 판정은 "무는 것이 있다/없다"만 말하지 **어느 집합이** 움직였는지는 안 말한다
  (래칫이던 시절에도 "늘었다/줄었다"까지였다).
