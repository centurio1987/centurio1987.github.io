...
- **게이트는 하드월이 아니라 래칫이다** — 커밋된 기준선 대비 **늘면 실패**한다. 부채가 네
  ...
  그때 하드월로 승격한다 — **KAN-072 와 KAN-074 가 두 번 0 을 만들었지만 승격은 두 번 다
  미뤄졌다.** 승격 조건은 「래칫 0」이 아니라 **「인식층이 다 열린 뒤의 래칫 0」**이다:
  KAN-073 이 인식층 셋을 열자마자 0 이 778 로 다시 섰고, KAN-075 가 넷째를 열자 다시 비0 이
  됐다. 아직 안 연 갈래가 **둘** 남아 있다(KAN-076 선 굵기 · KAN-077 상수 승격).
  그 둘이 닫히기 전에 올리면 두 카드가 **CI 를 빨갛게 만들면서 착수**하게 되고 그동안 배포가 멈춘다.
...
- **게이트의 「0」은 인식 범위 안에서의 0 이다 (KAN-073).** KAN-072 가 래칫을 0 으로 만들자마자
  ...
  **똑같은 코드로** 판정되므로 사유가 겹친다). 아직 안 여는 것 둘은 진단 정본 §6 의 12·13항이다.
...
- **화면이 안 바뀌었음은 눈이 아니라 `bun run render:compare` 가 증명한다 (KAN-074).**
  ...
  **직접 짜면 조용히 통과한다** — 인라인 hex 를 셀렉터로 세면 React 가 rgb() 로 정규화한 노드가
  ...
  셋 다 실측으로 밟았고 `--self-test` 가 고장 3종 + 잡음 0 으로 매번 확인한다. 시간에 끌려다니는
