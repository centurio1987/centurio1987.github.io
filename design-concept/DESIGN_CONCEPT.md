...
`--text-small` 을 세운 것과 같은 경위이고, 셋 다 **구멍이지 설계가 아니다** — 글 안 시뮬 6편이
...
전용**이라고 못박은 배지 색이다. **최근접이 곧 정답이 아니다 — 역할이 먼저다**(§5 대전제와 같다).

**서로를 구분하는 색표는 여기 넣지 않는다.** 프레임 필드 6색이나 상태 테두리 4색처럼 「항목을
서로 가르는」 용도의 색은 시맨틱 이름을 붙일 자리가 아니라서 토큰으로 세우지 않았다(유저 판정
2026-08-27). 그런 자리는 게이트가 계속 물게 두고 수렴 단계에서 함께 본다.
