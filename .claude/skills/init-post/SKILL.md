...
> **대화 원본이 있으면 이 경로가 아니라 `talk-ingest` 다.** 여기는 원본 없이 처음부터 쓸
> **빈 스켈레톤**을 만드는 자리이고, AI 대화 export 를 넣어 에피소드를 만드는 것은
> `talk-ingest` 가 3계층(원문·정형본·발행물)으로 굽는다. 둘 다 아래 템플릿을 공유하지만
> 진입점이 다르다 — 원본이 있는데 여기로 오면 그 원본이 보존되지 않는다.
