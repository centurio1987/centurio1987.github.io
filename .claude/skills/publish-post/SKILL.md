...
스키마 기본값은 `ppangto`(사람 저자)다. 즉 **`author`를 안 적으면 AI가 쓴 글이 사람 이름으로 발행된다.**
`src/lib/authors.ts` 의 id 중 하나를 **반드시 명시**한다:

| id | 누구 | 언제 |
| --- | --- | --- |
| `ppangto` | 빵관 토니 (사람) | 사람이 직접 쓴 개인 회고·에세이·사이트 공지 |
| `ppangto` | 빵토 연구원 (AI) | AI가 완전 집필한 기술 심층 시리즈 (기본값에 가장 가깝다) |
| `ppangtolab-prof` | 빵토랩 교수님 (AI) | 폭신 대담 |
| `ppangtolab-teacher` | 빵토랩 선생님 (AI) | 자료구조·알고리즘 강의 톤 |
...
- **"○○의 해부" 같은 제목 패턴은 저자 단서가 아니다** — `ppangto` 와 `ppangtolab-teacher` 가 모두 쓴다.
...
- **`author` 를 비워 두지 않는다.** 스키마 기본값이 `ppangto`(사람)라서, 비우면 AI가 쓴 글이
