#!/usr/bin/env bash
# Send a technical article (md/mdx) to the codex CLI (ChatGPT) and the antigravity
# CLI (agy / Gemini) in parallel and print both reviews. Mirrors the structure of
# ~/.claude/scripts/review-plan.sh (auth/skip/timeout/parallel) but uses a 3-axis
# article-review prompt: omissions, contradictions, factual accuracy.
#
# Usage:
#   review-article.sh <article-file>
#
...
# gate should treat the draft as NOT review-passed).
#
...
if [ -z "$ARTICLE_FILE" ]; then
  err "사용법: review-article.sh <article-file>"
  exit 1
fi
if [ ! -r "$ARTICLE_FILE" ]; then
  err "글 파일을 읽을 수 없습니다: $ARTICLE_FILE"
  exit 1
fi
if [ ! -s "$ARTICLE_FILE" ]; then
  err "글 파일이 비어 있습니다: $ARTICLE_FILE"
  exit 1
fi

ARTICLE_BODY="$(cat "$ARTICLE_FILE")"

SYSTEM_PROMPT='너는 깐깐한 기술 문서 검토자다. 주어진 기술 글의 약점을 한국어로 지적하라.'
USER_PROMPT_HEAD='다음 기술 해설 글을 검토해 줘. 아래 세 축을 중심으로, 가장 중요한 지적부터 항목별 bullet로 한국어로 답해 줘.

1) 누락/생략: 주제를 제대로 이해하는 데 꼭 필요한데 빠진 내용(개념·단계·장비·프로토콜·예외 케이스·전제). "이건 더 깊이 들어가야 한다"는 곳을 구체적으로.
2) 모순/충돌: 글 안에서 앞뒤가 어긋나는 서술, 정의와 사용이 다른 용어, 그림/표와 본문의 불일치.
3) 사실 정확성/시대성: 틀렸거나 낡은 설명, 표준/RFC/스펙과 어긋나는 수치·동작, 근거가 약한 단정.
...
if [ "$ANY_OUTPUT" -eq 0 ]; then
  err ""
  err "두 모델 모두 응답을 받지 못했습니다 — 외부 검토 미통과. draft를 '완성'으로 간주하지 마세요."
  exit 2
fi
