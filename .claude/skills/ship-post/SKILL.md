...
- **인용 문단 하드 게이트**: `bun scripts/check-quote-blocks.ts` 실행 → 인용 한 문단에 「둘째」·「셋째」가 이어 붙어 있으면 **종료코드 1**. 「읽기 전 약속 N」 프리앰블이 벽 하나로 읽히던 결함이고(vpn-anatomy-6 이 564자 한 문단으로 나갔다), 마크다운 문법은 멀쩡해서 빌드·타입·강조 게이트가 전부 초록이다. 고치는 법은 표지 앞에 `>` 만 있는 빈 줄 하나 — **글자는 안 바꾼다.** 300자 넘는 한 문단 인용은 보고만 하므로 사람이 판단한다. (CI `main.yml` 도 build 전에 같은 가드를 돌린다.)
