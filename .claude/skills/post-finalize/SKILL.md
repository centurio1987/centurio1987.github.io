...
description: >
  ...
  frontmatter `series` 가 있으면 같은 시리즈 다른 글의 링크 목록을 본문 하단에 삽입하며,
...
3. `<<meme: ...>>` 마커가 남아 있으면 `meme-inserter` 에 위탁
4. 잔존 레거시 마커(`[[[...]]]`·```figure```·`(( ))`) 검사 → 경고
...
- 시리즈 링크 경로: `/posts/<slug>` (Astro 라우팅, 확장자 제거)
...
1. `src/content/posts/` 전체를 글롭(`Glob` + frontmatter 파싱)으로 훑어 같은 `series` 값을 가진 `.md`/`.mdx` 를 모은다.
2. 현재 파일 제외.
