---
name: graph-pipeline
description: 연관 글·`/graph` 글 지도를 떠받치는 graphify 지식그래프 파이프라인의 운영 절차와 함정. 그래프를 재생성하거나(`bun run graph:refresh`), 자동 갱신 훅·예약제를 손보거나, 증분 캐시·검증(`verify-graph.ts`)·노드 ID 충돌·`/graph` 뷰 규약을 다룰 때 읽는다. 실측값과 "왜 이렇게 됐는가"가 들어 있어 추측으로 고치면 안 되는 영역이다.
---

# graphify 그래프 파이프라인

> 이 문서는 루트 `CLAUDE.md` 의 **Graph data** 절에서 분리해 온 것이다.
> 루트에는 어겨서는 안 되는 금지 사항과 계약만 남기고, 절차·근거·실측값은 여기 있다.
> 둘이 어긋나면 **루트의 금지 사항이 이긴다.**
