 ...
 * 게이트 대상에서 빼는 자리.
 *
 * **빼는 것이 아니라 표시만 한다.** 추출 결과가 감사와 같은 집합이어야
 * 히트 단위 대조가 성립하기 때문이다. 총계는 「전체」와 「게이트 대상」 두 벌로 낸다.
 */
export const EXCLUSIONS: { test: (file: string, text: string) => boolean; reason: string }[] = [
  { test: (f) => f === "src/styles/motion.css",
    reason: "생성물 — scripts/gen-motion-css.ts 가 굽는다. 정본이 TS 라 방향이 반대다" },
  { test: (f) => f === "src/styles/viz.css",
    reason: "패키지 복제물 — @centurio1987/bbangto-ui-visualization 의 CONTRACT_CSS 사본이다" },
  { test: (_f, t) => t.includes("AUTO-GENERATED by scripts/apply-viz.ts"),
    reason: "생성물 — apply-viz 가 굽는다. 좌표를 건드리면 다이어그램 기하가 깨진다" },
];
...
export function extract(root: string): { files: string[]; dict: TokenDict; hits: Hit[] } {
  ...
  for (const rel of files) {
    ...
    const ex = EXCLUSIONS.find((e) => e.test(rel, text))?.reason ?? null;
