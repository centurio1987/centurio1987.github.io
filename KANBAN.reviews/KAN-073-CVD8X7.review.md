...
- [x] 새 기준선을 비0(위반 170 · 드리프트 608)으로 세운 것에 동의하는가 — 카드 목표대로 세웠다
...
- [x] 갈래 C 의 width:0·height:0 이 준수가 아니라 판정 불가로 남는 것에 동의하는가 — 규칙을 안 비틀었다
...
- [x] 계획 수치 셋이 실측에서 갈린 것을 그대로 받은 판단에 동의하는가 — 셋 다 규칙을 안 비튼 쪽이다
...
- [x] 자가검사 유일성 키를 (판정, 사유, 인식 경로) 셋으로 넓힌 것에 동의하는가 — 느슨하게 푸는 대신 키를 늘렸다
...
- [x] 남긴 넷을 후속 카드로 올릴 것인가 — 올리는 쪽을 제안한다
...
- [x] 계획 리포트의 「루트 독립성 겹침 0」을 그대로 둘 것인가 — 조건을 함께 적었다
