...
- [x] 기준선 파일에 정보 집계(준수 4,251 · 판정 불가 894 · 정당한 예외 14)를 계속 두기로 했다 — 동의하는가
...
- [x] -update-baseline 을 이름은 그대로 두고 「위반이 있으면 거부」로만 막았다 — 동의하는가
...
- [x] 판정 불가 894건이 위반의 도피처가 되는 것을 「실패로 물지 않고 가르기만」 하기로 했다 — 동의하는가
...
- [x] CI 워크플로를 한 글자도 안 고쳤다 — 하드월이 배포를 멈추는 것을 그대로 둔다. 동의하는가
...
- [x] 자가검사에 「판정 방식 검사」 넷을 픽스처가 아닌 방식으로 넣었다 — 규약에서 벗어난 것이 맞는가
...
- [x] 착수 전에 main 에 커밋 하나(ae5949c)를 직접 밀었다 — 사후 승인을 청한다
