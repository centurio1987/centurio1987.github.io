...
- [x] SAND 5자리를 var(--field-dst) 로 모은 것이 맞는가 — 값은 똑같은데 그 토큰의 뜻이 다르다
    ...
    > **추가 의견**
    >
    > - 유저 · 2026-08-30 — 값은 본질이 아니다. 의미 기반으로 나눠야 한다.
    > - ai · 2026-08-30 — 반영했습니다. 값으로 묶던 것을 걷고 두 역할에 자기 이름을 세웠습니다 — SAND 5자리는 --state-plain(#e8c97a, 「평문·캐시 미스」, 보호됨/적중은 --cat-skills), MUTED 5자리는 --state-inactive(#b8b0a0, 「비활성·조작 불가」). 값이 그대로라 화면 변화 0 이고 render:compare 가 46쪽 전수에서 지면 0곳·0건으로 증명했습니다. 지적을 그 자리 하나로 안 끝냈습니다 — SAND 만이 아니라 MUTED 와, 이미 같은 식으로 --fate-blocked-border 에 묶여 있던 HopJourney.tsx:213 까지 함께 옮겼습니다. 값이 같은 토큰이 둘씩 생기지만(--field-dst / --fate-blocked-border) 그것이 요점입니다: 나중에 프레임 필드 색표나 트래픽 상태 색표를 다시 고를 때 이제 **어느 쪽을 고칠지 고를 수 있습니다.** 게이트는 안 뭅니다(색 축 토큰 108→110, fallback 불일치 0, 기준선 불변). 문서도 갱신했습니다 — tokens.css 의 구분 팔레트가 18종→20종이 됐고 DESIGN_CONCEPT.md §4 표에 두 줄과 「왜 값이 겹치는데도 이름을 나누나」 문단을 넣었습니다. KAN-072 가 --state-* 를 안 세운 근거(같은 값에 이름이 둘)와 어긋나 보이는데, 그 카드가 본 것은 값도 역할도 같은 자리였고 여기는 역할이 갈리는 자리라는 차이를 양쪽 문서에 적었습니다. **이름 두 개는 제가 정할 것이 아니라 판정 대상입니다** — --state-plain · --state-inactive 로 괜찮은지, 아니면 다른 이름이 좋을지 알려 주십시오.

- [x] TEAL_TINT 5자리를 기존 관용구로 모은 것이 맞는가 — 발행된 글 두 편의 배경색이 실제로 바뀐다
...
- [x] 인식기가 굳힌 규칙 둘 — 「선언 줄을 지목한다」와 「숫자도 본다」
...
- [x] render:compare 가 대조를 안 하고도 「전후 동일」이라고 답한다 — 어떻게 처리할까
    ...
    >
    > - 유저 · 2026-08-30 — 새카드
    > - ai · 2026-08-30 — 새 카드로 내겠습니다. 다만 **이 브랜치에서 세우지 않습니다** — 루트 카드는 main 에서 추가한다는 규율이 있고(add 가 종료코드 7 로 막습니다), 브랜치에서 우회하면 .kanban/state.json 과 .kanban/log.md 가 양쪽에서 갈려 병합 때 손으로 풀어야 하는데 log.md 는 손실 렌더라 rebuild 로도 못 되살립니다. 그래서 **카드 본문을 확정해 두고 이 카드가 main 에 병합된 직후에 세웁니다.** 확정한 내용 — 제목 「render:compare 가 대조를 안 하고도 「전후 동일」이라고 답한다 — 거짓 통과를 막는다」 · 목적: 화면 무변경을 증명하는 도구가 대조를 건너뛰고도 통과로 끝나는 자리를 막는다 · 이유: KAN-077 에서 실제로 밟았다(「전후 동일」을 받은 뒤 옵션만 빼고 다시 돌리니 28건) · 목표: 자가검사만 돌린 실행이 「전후 동일」로 끝나지 않고, 같은 거짓 통과가 다시 나면 검사가 문다. 메모에는 자리(scripts/compare-render.ts:667 의 if/else 와 파일 끝의 console.log)와 갈래 후보 둘(①자가검사 뒤에도 실제 대조를 이어서 돈다 ②자가검사만 돌았으면 마지막 줄을 「자가검사 통과(대조 안 함)」로 가른다)을 넣었고, **고치는 것으로 끝내지 않고 그 거짓 통과를 재현하는 자가검사를 함께 둔다**는 것도 적었습니다(KAN-076 S8 이 selfTestFaults 로 같은 종류를 막은 선례). 카드 id 는 세운 뒤 보고하겠습니다.

- [x] 하드월 승격 — 조건이 이 카드로 처음 충족됐다. 다음에 할 것인가
