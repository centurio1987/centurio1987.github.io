...
- [x] `S7` 부채 15건 처분 — 세 갈래를 재고 하나를 고른다
...
- [x] `S9` 검토 1항 지적 반영 — 값이 아니라 **의미로** 나눈다
      검토자 지적: 「값은 본질이 아니다. 의미 기반으로 나눠야 한다.」 `S7` 은 값이 같다는
      이유로 `SAND` 를 `--field-dst`(2화 목적지 MAC 칸)에, `MUTED` 를 `--fate-blocked-border`
      (VPN 7화 차단됨)에 묶었다 — 한 이름이 두 뜻을 지게 되는 자리다.
      **지적은 인스턴스가 아니라 클래스다** — `SAND` 만이 아니라 `MUTED` 와, 이미 같은 식으로
      묶여 있던 `HopJourney.tsx:213` 까지 함께 본다.
      완료 기준: ① 두 역할에 **자기 이름**이 서고(값은 그대로라 화면 변화 0) ② 그 이름이
      `tokens.css` 와 `DESIGN_CONCEPT.md` §4 구분 팔레트 표에 함께 오르며 ③ 같은 값을 든 토큰이
      둘이 되는 것을 게이트가 어떻게 보는지 확인하고 ④ `render:compare` 로 화면 무변경을
      증명한다. ⑤ 이름 자체는 검토서 1항에 추가 의견으로 되돌려 유저가 판정한다.

- [x] `S10` 검토 4항 지적 반영 — `render:compare` 결함을 새 카드로 낸다
      검토자 지적: 「새카드」. `--self-test` 가 전후 대조를 한 번도 안 하고 「✓ 전후 동일.」을
      찍는 결함이고, 고칠 파일이 이 카드 scope 밖이다.
      **카드를 이 브랜치에서 세우지 않는다** — 루트 카드는 `main` 에서 추가한다는 규율이 있고
      (`add` 가 종료코드 7 로 막는다), 브랜치에서 우회하면 `.kanban/state.json` 과
      `.kanban/log.md` 가 양쪽에서 갈려 병합 때 손으로 풀어야 한다(`log.md` 는 손실 렌더라
      `rebuild` 로도 못 되살린다). 그래서 **카드 본문을 여기서 확정하고, 세우는 것은
      이 카드가 `main` 에 병합된 직후**에 한다.
      완료 기준: ① 카드 제목·브리프 넷·메모·원문이 확정되어 이 문서의 수행 내역에 그대로
      들어간다 ② 그 본문과 「병합 직후 세운다」는 사실을 검토서 4항에 추가 의견으로 되돌린다
      ③ 병합 뒤 `main` 에서 `add` 를 실행하고 카드 id 를 보고한다.
...
- 2026-08-30T15:25 · s:0a6f09b5 · S7 doing — 착수
- 2026-08-30T15:42 · s:0a6f09b5 · S7 done — 부채 15건 처분 — 래칫 15 → 0. 세 색의 역할이 5자리씩 완전히 일관돼 있었다: MUTED=비활성 버튼 글자(color: i===0 ? MUTED : INK + cursor:not-allowed) · SAND=평문/비캐시 상태 테두리(TEAL 의 짝) · TEAL_TINT=활성/암호화 배경 틴트. 갈래는 색마다 다르게 골랐다. ①MUTED 5건 → var(--fate-blocked-border, #b8b0a0): 값 Δ0 이고 역할도 같으며(「차단됨(침묵)」↔「비활성」) **같은 레포에 이미 그 치환을 한 자리가 있다**(HopJourney.tsx:213). ②SAND 5건 → var(--field-dst, #e8c97a): 값 Δ0. 여기는 갈등이 있다 — 값이 같은 토큰(--field-dst=Destination MAC)은 역할이 다르고, 역할이 같은 토큰(--fate-direct-border=「평문 직결」)은 Δ24.4 라 배치5 규약(15 이상은 임의로 고르지 않는다)에 걸린다. 값 동일 쪽을 골랐다 — KAN-072 S11 이 const OK=#3E6B4F → --cat-planning 에서 같은 판단을 했고(역할이 달라도 값이 같으면 var() 치환), 새 토큰은 KAN-072 가 --state-* 를 세우지 않기로 한 근거(같은 값에 이름이 둘)에 걸린다. **커플링은 검토 판단 항목으로 올린다.** ③TEAL_TINT 5건 → color-mix(in srgb, var(--cat-skills) 11%, var(--surface-hi)): 토큰이 없고, 이 관용구가 **같은 시리즈에 이미 7자리**(EP1 EncapsulationLab·LayerTriage, EP2 SwitchLearningLab, EP3 HopJourney:110·RoutingTableLab:132/160)와 auth-authz 4편에 쓰이는 확립된 활성 틴트다. Δ7.7 이라 화면이 조금 바뀐다. render:compare 실측: 지면 4곳 중 EP5·EP6 두 편에서 **background-color 28건**만 움직였고(rgb(229,240,237)→rgb(234,237,232)) 문서 높이·기하 변화 0. EP3 은 TEAL_TINT 가 이미 6% color-mix 라 무변경, EP4 는 하네스가 active 상태에 도달 못 해 안 잡혔을 뿐 같은 값이 걸려 있다. 스테일 주석 6개(「최근접 --pop-tint ΔRGB 50」 등, KAN-072 가 나중에 세운 --field-*·--fate-* 를 안 본 수치)를 새 근거 주석으로 갈았다. 기준선 재갱신: 드리프트 10·위반 5 → 0, 준수 3639→3649, fallback 자리 521→531
- 2026-08-30T15:42 · s:0a6f09b5 · S6 doing — 착수
- 2026-08-30T15:45 · s:0a6f09b5 · S6 done — 문서 셋. ①감사 정본 UI_CONSISTENCY_AUDIT.md §6 에 **14항 신설** — 상수 승격 항이 아예 없던 자리다(§6-9·§6-10 과 같은 경위). 담은 것: 회피 경로였다는 것과 KAN-072 S10 의 152 대 6 전례 · 실측 15/3/8 과 카드 메모의 6파일·45참조 정정 · 사용처 추적이 기법인 이유 · 범위 한정이 전부인 이유(89 중 71이 값 자리 밖) · 히트를 선언 자리에 내는 규칙 · 숫자를 지금 0건인데도 담는 이유 · S7 처분. ②CLAUDE.md — 인식층 문단에 여섯째 갈래를 넣고 「아직 안 연 갈래 하나」를 **없앴다**. 하드월 승격 문단도 고쳤다: 승격이 네 번 미뤄졌다는 사실과, **KAN-077 로 조건이 처음 충족됐다**는 것(열 갈래가 없고 래칫 0), 다만 승격 자체는 CI 를 빨갛게 만들 위험을 지므로 인식층 커밋과 안 섞는 별건이라는 것. §6 13항이 '안 여는 것'이 아니라 '0건이라 열 것이 없는 자리'임도 명시. ③소스 주석 8파일 — 스테일 6개를 갈고 SAND 단독 2파일에 새로 달았다. 문구를 한 번 고쳤다: 처음 쓴 '리터럴을 const 로 올려 두지 않는다'가 이제 낡은 조언이라(게이트가 무니 상수로 묶는 것 자체는 문제가 아니다) '상수에는 리터럴이 아니라 토큰 참조를 담는다'로 바꿨다. 세 문서가 같은 수(15·3·8·89/71)를 쓴다
- 2026-08-30T15:45 · s:0a6f09b5 · S8 doing — 착수
- 2026-08-30T15:47 · s:0a6f09b5 — `검증` 섹션 교체
- 2026-08-30T15:48 · s:0a6f09b5 · S8 done — 검증 9줄 전건 통과, 결과를 「검증」 절 표에 실었다. 두 가지를 기록해 둔다. ①**2번(불변 증명)의 기준점이 HEAD 가 아니라 S5 커밋(f49dd80)이다** — S7 이 그 뒤에 src/ 값을 일부러 바꿔서 HEAD 에서 돌리면 ①이 '빠짐 363·늘음 373'으로 빨개지는데 그것은 고장이 아니라 부채를 갚은 흔적이다. detached 워크트리로 f49dd80 을 꺼내 재현했고 통과한다. 재현 명령을 표 밑에 적었다. ②**render:compare 의 --self-test 모드가 대조를 안 하고도 「✓ 전후 동일」을 찍는다** — SELF_TEST 가 실제 대조와 if/else 로 갈리는데(compare-render.ts:667) 마지막 줄은 공통이라, 자가검사만 돌린 실행이 '전후 동일'로 끝난다. 실제로 이 카드에서 그것을 밟았다: --self-test 로 돌려 '전후 동일'을 받았지만 --self-test 없이 다시 돌리니 28건이 나왔다. 게이트가 조용히 통과하는 자리라 이 카드의 주제 그 자체인데 **compare-render.ts 는 이 카드 scope 밖이므로 안 고쳤다** — 검토 판단 항목으로 올린다. 그 밖에 인식기가 지금 레포에서 히트 0 이 됐다(부채를 다 갚아서) — 죽어도 레포로는 못 보는 상태이고, 그래서 고장 픽스처 둘과 selfTestFaults 래칫이 유일한 생존 증거다
- 2026-08-30T21:06 · s:0a6f09b5 — `실행 계획` 섹션 교체
- 2026-08-30T21:07 · s:0a6f09b5 · S9 doing — 착수
- 2026-08-30T21:19 · s:0a6f09b5 — `실행 계획` 섹션 교체
- 2026-08-30T21:44 · s:0a6f09b5 · S9 done — 검토 1항 반영 — 값이 아니라 의미로 나눴다. 토큰 2종 신설: --state-plain #e8c97a(평문·캐시 미스, 보호됨/적중은 --cat-skills) · --state-inactive #b8b0a0(비활성·조작 불가). 값은 그대로라 화면 변화 0 이고, 그것을 render:compare 가 증명했다 — tokens.css 가 바뀌어 범위 유도가 전 글 대조로 넓어졌고 **46쪽 전수에서 지면 0곳·0건**. **지적을 인스턴스로 안 끝냈다** — SAND(5자리) 뿐 아니라 MUTED(5자리)와, 이미 같은 식으로 묶여 있던 HopJourney.tsx:213 의 var(--fate-blocked-border) 까지 함께 옮겼다. 값이 같은 토큰이 둘씩 생기는데(--field-dst / --fate-blocked-border) 게이트는 안 문다 — 색 축 토큰 108→110 으로 늘고 fallback 자리 531→532, 불일치 0, 기준선 불변. 문서 셋 갱신: tokens.css 머리(18종→20종)와 새 그룹 주석 · DESIGN_CONCEPT.md §4 에 표 한 칸과 「왜 값이 겹치는데도 이름을 나누나」 문단. KAN-072 가 --state-* 를 안 세운 근거(같은 값에 이름이 둘)와 어긋나 보이지만 그 카드가 본 것은 값도 역할도 같은 자리였고 여기는 역할이 갈리는 자리다 — 그 차이를 tokens.css 주석과 DESIGN_CONCEPT 양쪽에 적었다. **이름 자체는 내가 정할 것이 아니라 검토 대상**이라 1항에 추가 의견으로 되돌린다
- 2026-08-30T21:45 · s:0a6f09b5 · S10 doing — 착수
- 2026-08-30T21:45 · s:0a6f09b5 · S10 done — 검토 4항 반영 — 새 카드 본문을 확정했다. 세우는 것은 이 카드가 main 에 병합된 직후다(루트 카드는 main 에서 추가한다는 규율. 브랜치에서 우회하면 .kanban/log.md 가 양쪽에서 갈리는데 그건 손실 렌더라 rebuild 로도 못 되살린다). 확정한 본문 — 제목: 「render:compare 가 대조를 안 하고도 「전후 동일」이라고 답한다 — 거짓 통과를 막는다」 · 짧은 제목: compare-render 거짓 통과 · 목적: 화면 무변경을 증명하는 도구가 대조를 건너뛰고도 통과로 끝나는 자리를 막는다 · 이유: KAN-077 에서 실제로 밟았다(전후 동일을 받은 뒤 옵션만 빼고 다시 돌리니 28건) · 목표: 자가검사만 돌린 실행이 「전후 동일」로 끝나지 않고 같은 거짓 통과가 다시 나면 검사가 문다 · 메모: scripts/compare-render.ts:667 의 if/else 로 SELF_TEST 갈래와 실제 대조가 갈리는데 파일 끝의 console.log("✓ 전후 동일.") 이 두 갈래 공통이다. 고칠 것은 한 줄이지만 **고치는 것으로 끝내지 않는다** — 그 거짓 통과를 재현하는 자가검사를 함께 둔다(KAN-076 S8 이 selfTestFaults 로 같은 종류의 자기 축소를 막은 선례). 갈래 후보 둘: ①자가검사 뒤에도 실제 대조를 이어서 돈다 ②자가검사만 돌았으면 마지막 줄을 「자가검사 통과(대조 안 함)」로 가른다 · 원문: 새카드
