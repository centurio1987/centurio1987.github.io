...
- `KAN-055` 포스트 글을 들어가는데 프리징이 생긴다. ux를 해치지 않기 위해 fallback loading ux를 추가하려고 한다. 프리징 원인을 진단하고, loading ux를 추가하는 것이 맞는 솔루션인지 검증 한 후, 최적의 솔루션을 채택하여, 수행 전략을 구상해라. — 생성:유저 · 최종:유저 · 갱신:2026-07-29
