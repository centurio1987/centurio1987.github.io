...
- `KAN-053` 백엔드 없이도 회원가입 기능을 추가할 수 있는지 알려주고, 가능 하다면 계획하고 수행해라. — 생성:유저 · 최종:ai · 갱신:2026-07-30
  - 메모: 유저 결정(2026-07-30): 댓글·회원가입 포기. 근거 — 승인제 가입 자체는 Supabase 로 가능하나(Before User Created 훅이 Free 플랜, Postgres 함수로 구현) 내장 메일러 2통/시간 제약과 UGC 렌더가 새로 여는 XSS 표면(정적 호스팅이라 BFF 불가, 세션이 localStorage)이 개인 블로그의 이득 대비 큼. 좋아요·공유 카운트만 KAN-048 로 진행.

## 할 일
- `KAN-057` 데코 키트를 블로그 실제 페이지에 적용 (홈 히어로 · 글 목록 · 글 상세 · 글 지도) — 생성:ai · 최종:ai · 갱신:2026-07-29
  - 메모: 부품·패턴·쇼케이스(/design/deco)는 자산화 완료 — 남은 건 실제 페이지에 얹는 일. 페이지별 강도(data-deco 1~5)를 정하고 해당 페이지에서만 src/styles/deco.css 를 import 한다(전역 금지 — opt-in 계약). 적용 후보: ① 홈 히어로를 P-02 HeroCollage 로 교체(지금 구워진 public/tony-deco.webp 를 대체해 말풍선이 텍스트가 됨 = 효과 최대) ② 홈·목록 섹션 머리에 P-06 DecoHeading + 최신 글 1건에만 P-07 도장 ③ 목록 필터 카드에 P-03 ClippedCard ④ 글 상세 인용 P-04 / 코드블록 P-05 / 시리즈 하단 P-08 ⑤ 글 지도 판에 P-01 TapedBoard. 주의: PostList 에 이미 시리즈 테이프(.series-tape)가 있어 목록에 테이프를 더 얹으면 중복이므로 그 자리는 피한다. 배치·접근성 규칙은 design-concept/DECO_KIT.md 5·6절.
  - 원문:
    ```text
