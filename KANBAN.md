...
- `KAN-032` 프로그래밍 레벨에서 async/await를 가능하게 만드는 원리 시리즈 집필 — 생성:유저 · 최종:ai · 갱신:2026-07-28
  ...
  - 원문:
    ...
    ```
- `KAN-059` 메인 화면의 이미지가 다른 요소들보다 늦게 뜨는 경우, 사용자 경험이 불쾌해진다. 배포 환경에선 늦게 뜨는 현상이 잘 나타난다. 이를 해소하기 위해 성능 측면에서의 전략과 ux 측면 전략(fallback loading 이라던가)을 구상하라. — 생성:유저 · 최종:유저 · 갱신:2026-07-31
