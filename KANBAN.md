...
## 할 일
- `KAN-024` graphify 시리즈 글 노드 ID 충돌로 문서가 그래프에서 누락되는 문제 해결 — 생성:ai · 최종:ai · 갱신:2026-07-27
  - 메모: KAN-008 발행 중 발견. 증상: graphify extract 가 청크 실패 0건으로 성공 보고하는데도 문서 노드가 글 수보다 적다(2026-07-27 기준 18/22). 누락: osi-7-layers-3·4·5·6. 원인: 시리즈 글이 본문에서 서로를 참조하면 osi-7-layers-1.mdx 가 osi_7_layers_3 같은 참조 노드를 먼저 만들고, 이후 osi-7-layers-3.mdx 의 진짜 문서 노드가 같은 ID 로 충돌해 "the second node will be dropped" 로 버려진다. 시리즈가 길수록·상호링크가 많을수록 악화. auth-authz·tauri 계열도 동일 충돌 발생. graphify 자체 경고문이 해법을 제시: 하위 폴더별로 extract 후 graphify merge-graphs 로 병합. 할 일: ① 시리즈별(또는 폴더별) 분할 추출 + merge-graphs 파이프라인을 scripts 로 구성 ② ship-post 1.5 단계와 CLAUDE.md 절차를 그 방식으로 교체 ③ 재생성 후 2단계 검증(문서 노드 수 = 글 수, 노드 1개짜리 글 없음) 통과 확인 후 커밋. 참고: 검증 절차는 a058700 에서 2단계로 보완해 둠 — 이 문제를 잡아낸 게 그 ① 검사다. 현재 영향: 연관 글은 frontmatter 랭킹으로, /graph 는 일반 목록으로 폴백하므로 사이트 동작에는 지장 없음(기능 degrade).
  - 원문:
    ```text
...
- `KAN-008` OCI, CRI — 생성:ai · 최종:ai · 갱신:2026-07-27
  - 메모: 시리즈 4편 "컨테이너의 해부" 발행 완료(저자: ppangto-teacher 빵토 선생님, 퍼소나 첫 적용). EP1 도커 해부(8632f75) / EP2 OCI 3대 스펙(4dbc311) / EP3 CRI(5ab2da5) / EP4 대안 다면 비교·완결(98ff6f1). 브랜치 centurio1987/kan-008 푸시, main 미머지(사용자 리뷰 대기). 편당 파이프라인 전 구간 통과: research→tech-deepdive(외부검토 codex/agy)→react-sim→make-image→review-post→review-writing→quality-gate→post-finalize→publish-post→ship-post. 산출물: 글 4편 + React 시뮬 3종(OverlayLab·DigestChain·CriCallTrace) + viz 14종 + hero 4장, 전부 브라우저 육안·상호작용 검증. 리서치: ~/blog-research raws/008 + 위키 10p + angle 4개(306f255). 검증에서 잡은 실오류: whiteout=캐릭터디바이스0/0(.wh.아님)·pstree 자기모순·copy_up 실습 무효·process.args만 보류(전체 아님)·Hyper-V root MUST NOT·prestart DEPRECATED·hero 제목 잘림. 부수 산출: CLAUDE.md graphify 검증 2단계화(a058700). 남은 것: 그래프 데이터 갱신, main 머지 여부 사용자 결정.
