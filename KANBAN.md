...
- `KAN-044` graphify 그래프 일괄 재생성 (VPN 시리즈 7편 전량 발행·머지 후 1회) — 생성:ai · 최종:ai · 갱신:2026-07-28
  - 메모: **KAN-045로 전제가 바뀌었다** — 근거 ①(매 실행 전량 재추출)은 원인이 규명·수정됐고, ②③은 "그래프 갱신은 main 워크트리 전용" 규약으로 해소됐다. 이 카드는 이제 "VPN 7편 모아서 1회"가 아니라 **KAN-045 부트스트랩 1회로 흡수**된다. 부트스트랩 후에는 main 머지 직후마다 `bun run graph:refresh`를 돌리면 되고, 바뀐 글만 LLM을 타므로 모아둘 이유가 없다. || 현재 상태: 커밋된 그래프는 22편(vpn-anatomy-1·4 누락)이라 `verify-graph.ts`가 ①③으로 실패한다 — 부트스트랩 전까지 연관 글·/graph는 frontmatter 폴백으로 degrade(기능은 동작). || 절차는 KAN-045 참조.
- `KAN-045` graphify 증분 추출 복구 — 글 1편 추가에 전수 재추출 중단 — 생성:ai · 최종:ai · 갱신:2026-07-28
  - 메모: **문제**: 글 1편 발행마다 24편 전량이 LLM 재추출됐다(실측 `graphify-out/.graphify_analysis.json`: 1,257,753 in / 279,644 out, claude-cli로 40~60분). N에 선형이라 편당 발행 시 누적 O(N²) — 50편이면 매 발행 ~2.6M 토큰. || **근본 원인(소스 확인)**: graphify 0.9.4는 콘텐츠 해시 캐시로 LLM 호출 전에 안 바뀐 글을 걸러내는데, 이 레포는 그게 두 겹으로 죽어 있었다. ⓐ `graphify-out/cache/`가 .gitignore(L23) — 캐시가 애초에 커밋 안 됨. ⓑ 그보다 근본적으로, **graphify 자신이 이 레이아웃에서 캐시를 한 건도 못 쓴다**: `save_semantic_cache()`가 `out_root/<source_file>` 실재 여부를 보는데 `source_file`은 스캔 루트(`src/content/posts/`) 기준 상대경로이고 `--out .`은 레포 루트라 경로 불일치로 통째 skip. 같은 절대/상대 불일치가 `_manifest_files`에도 있어 `manifest.json`은 문서 코퍼스에서 **항상 `{}`**(그래서 첫 커밋부터 계속 비어 있었다). || **해결**: `scripts/seed-graph-cache.py`가 추출 직후 `source_file`을 절대경로로 바꿔 graphify **자신의** `save_semantic_cache(root=레포루트)`에 넘겨 캐시를 심고, `check_semantic_cache()`로 전 글 적중을 자체 검증한다(해시는 graphify 함수 그대로 — 재구현 없음). manifest는 커밋하지 않는다(빈 파일이 있기만 해도 `incremental_mode`가 켜져 삭제 감지가 죽고 지운 글 노드가 영구 잔존). || **실측 검증(스크래치 프로브, 글 2편)**: 콜드 34,657 in / 6,168 out → 캐시 적중 시 **`semantic cache: 2 hit / 0 miss`, LLM 0콜, 토큰 0**, 그래프는 26노드·41엣지로 **동일**. 1편만 수정 시 **`1 hit / 1 miss`** — 바뀐 글만 재추출. || **산출물**: `scripts/refresh-graph.sh`(가드 3종: main 워크트리 전용·graphify 0.9.4 핀·캐시 기준선 확인) + `scripts/seed-graph-cache.py` + `verify-graph.ts` 검사 ④ 신설 + `.gitignore` 재작성(cache/semantic 커밋, cache/ast·stat-index·manifest·graph.html·logs 제외) + `package.json` graph:refresh/graph:verify + ship-post 2벌·CLAUDE.md·README 규약 동기화. || **남은 일**: main에서 부트스트랩 1회(콜드 40~60분, KAN-044 흡수) → 기준선 실측 4케이스(무변경/1편 본문/프론트매터만/시리즈 링크) 기록. || **주의**: `.mdx`는 프론트매터까지 해시된다(`cache.py`의 스트립은 `.md` 전용) — post-finalize의 tags·시리즈 링크 삽입이 형제 편 재추출을 유발한다. 또 claude-cli 백엔드는 청크당 입력 하한이 ~30k라 1편 추가도 완전 공짜는 아니다(전량 1.26M 대비 ~95% 절감).
...
- `KAN-034` VPN 해부 리서치② — 캡처 랩 환경·방법론 — 생성:ai · 최종:ai · 갱신:2026-07-28
  - 메모: 완료(검토 대기). 재현 가능한 캡처 랩 확정 — ~/blog-research/raws/010-vpn-anatomy-capture-lab.md(불변, 22KB) + 위키 topic vpn-capture-lab(mature) 신설, blog-research 커밋·푸시. 산출: Docker 2노드+NAT 토폴로지(underlay 192.0.2/203.0.113 · overlay 198.51.100, MASQUERADE로 NAT-T 유발, NET_ADMIN·ip_forward), 버전·암호군 고정(strongSwan 5.9.13-2ubuntu4 aes256gcm16 / WireGuard 커널6.8 내장+tools 1.0.20210914 고정암호군 / OpenVPN 2.6.x data-ciphers AES-256-GCM+tls-crypt), 프로토콜별 캡처 명령(tcpdump 필터+tshark -O <proto> -V -x)·복호 절차(ESP=ip xfrm state 키→Wireshark ESP SA 또는 교육용 null-sha256 / WireGuard=키 로그 4토큰, 완전복호엔 ephemeral 필요 / OpenVPN=제어채널 TLS·데이터채널 복호 불가·구조만 / IKE_SA_INIT 평문)·보안 마스킹(합성 키·RFC5737/3849 IP·RFC7042 MAC)·증거 파일명 규약 vpn-anatomy-<n>/<capture>.txt + EP↔캡처 매핑표. accept 충족(각 프로토콜 캡처 재생성 명령/노트 문서화). EP3~7(KAN-037~040) 집필 gate 해제. open questions 5건: IKE_AUTH SK_ 키 추출(strongSwan) 미확립 / WireGuard ephemeral 키 로그(wireguard-go 계측) 미확립 / OpenVPN 최신 패치 발행 직전 재확인 / AES-GCM ESP SA salt 배치 실검증 / 커널6.8+ 컨테이너 wg-quick sysctl RO 이슈.
