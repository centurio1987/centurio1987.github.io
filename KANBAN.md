...
- `KAN-080-WTQ7AV` 타이포그래피가 하나의 디자인 스타일 안에서 일관성 없이 정의되어 있다. 하나의 디자인 스타일을 준수하는 타이포그래피 조합을 조사하여 채택해라. — 생성:유저 · 최종:ai · 갱신:2026-08-31
  ...
  - 이유: 타이포 토큰이 폰트·크기 두 축뿐이라 행간 59자리(값 21종)·자간 44자리(px/em 혼용 20종)·굵기 5종·시뮬 raw monospace 35자리가 관리 밖이고, propAxis.ts 의 UNITLESS_PROPS 가 행간·굵기를 게이트 시야에서 빼 tokens:verify 는 0 이라고 답한다
