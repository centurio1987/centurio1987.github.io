...
- `KAN-059` 메인 화면 이미지가 늦게 뜨는 문제 — 성능·UX 전략 구상 — 생성:유저 · 최종:ai · 갱신:2026-07-31
  - 메모: 배포 환경(GitHub Pages)에서 더 두드러진다는 유저 관찰. 두 갈래로 구상한다 — ① 성능: 로딩 우선순위(fetchpriority·preload)·lazy 경계·포맷/용량·치수 예약 ② UX: fallback loading·플레이스홀더 등 늦게 떠도 덜 불쾌하게 만드는 장치. 착수 시 현행부터 잴 것 — 홈(/)이 지금 이미지 7장(히어로 다이컷 ×2 · /tony-face.webp · 푸터 저자 라인업 4장)을 싣고 그중 5장이 loading=lazy 다. 푸터 라인업은 모든 페이지에 있으므로 홈만의 문제가 아닐 수 있다.
  - 원문:
    ```text
