...
- `KAN-081-QBTAPJ` 빵토 대담 소재 — 생성:유저 · 최종:유저 · 갱신:2026-09-02
  - 원문:
    ```text
    빵토 대담 소재
    - 도메인 모델 관련 고민이다.
      - 첫 번째 주제
        - A라는 모델이 있다. 다수의 A에 대해 불변식을 적용해야 해서 AManager라는 모델을 새로 정의 했다. 하지만, AManager는 영속화 필요가 없고, 처리 맥락 안에서 하나만 존재하면 된다. 어쨌든 하나만 존재하면 된다는 자체로 식별성이 있다고 판단하면 될까? 그렇지 않다면 식별성이 필수인 Entity나 AR로는 정의할 수 없을 것 같은데 AManager는 도메인 모델에서 어떤 개념으로 상정할 수 있을까
      - 두 번째 주제
        - entity는 식별성이 있어야 한다. 그래서 id가 있어야 하는데, 실제 로직에서는 사용하지 않는 값이다. 그렇다면 굳이 entity에 id가 있어야할 필요가 있나
    ```
...
- `KAN-065` 문체 스윕 후 graph:refresh 1회 (graph.json 라벨 낡음) — 생성:ai · 최종:ai · 갱신:2026-08-04
  ...
  - 원문:
    ...
    ```
- `KAN-082-ER9NJ4` 빵토 대담 주제 추가 — 생성:유저 · 최종:유저 · 갱신:2026-09-03
  - 원문:
    ```text
    빵토 대담 주제 추가
    - 소켓을 사용해서 실시간으로 서버-클라이언트 데이터가 동기화 되어야 하는 경우, 서버에서는 데이터를 메모리에 띄워놓고, 클라이언트랑 소통할 것이다. 하지만 이 경우, 서버 장애에 대응하기 어렵다. 이에 대한 대처 방안을 알려줘. 또한, 세션이 늘어나면, 메모리 관리가 어렵다. redis 같은 캐시를 사용하면, 부담이 줄겠지만, 대신에 인스턴스 생성 비용이 발생한다. 모든 문제를 해결할 수 있는 최적의 솔루션이 있다면 알려줘.
    ```
- `KAN-083-1EEG2W` 발행 글 27편의 hero.webp 를 지금 서체·굵기로 다시 굽고, 이번에는 스펙을 레포에 남긴다 — 생성:ai · 최종:ai · 갱신:2026-09-03
  - 짧은 제목: hero 래스터 재생성
  - 목적: 발행 글 27편의 hero.webp 를 지금 서체·굵기로 다시 굽는다
  - 이유: 그림 안 글자가 합성 볼드로 굳어 있어 같은 글의 본문과 굵기가 다르게 보인다 — KAN-080 이 굽는 코드는 고쳤지만 이미 구워진 픽셀은 안 바뀐다
  - 목표: 27장이 새 파이프라인으로 다시 구워지고 그 스펙이 이번에는 레포에 남는다(사이드카 또는 글 안 블록)
  - 메모: KAN-080 검토서 5항 승인(유저·화면·2026-09-03)으로 분기한 카드다. 사양 정본은 design-concept/UI_CONSISTENCY_AUDIT.md:1136 (7-11 절) — 그대로 등록하라고 적혀 있다. 실측: 발행 글에 잔존 viz 블록 0건 · 사이드카 0건 · 커밋 이력 전체에 hero 스펙을 담은 파일이 미발행 draft 하나뿐(draft/pagination-anatomy-1.mdx). scripts/apply-viz.ts:163 이 저작 시점에 viz 블록을 이미지 링크로 치환해 소비했고 그 일이 draft/ 단계라 커밋에 안 남았다 — 그래서 되굽기가 「27장 스펙 재작성」이다. 범위: public/images/*/hero.webp · scripts/render-viz.ts · scripts/apply-viz.ts(스펙을 소비하지 않고 남기도록). 교훈이 목표에 박혀 있다 — 스펙이 없어 되굽기가 재작성이 된 것이 이 분기의 원인이다.
  - 원문:
    ```text
    대표 그림 27장이 옛 굵기로 굳어 있는 것을 후속 카드로 미루는가 — 그림 파일로 이미 구워져 이번 변경이 안 닿습니다. 추천: 미룬다
    → 승인 · 유저 · 2026-09-03 (KAN-080-WTQ7AV 검토서 3항 5번)
    ```
...
## 검토
- `KAN-079-1QVHJ2` 토큰 게이트를 래칫에서 하드월로 승격한다 — 조건이 KAN-077 로 처음 충족됐다 — 생성:ai · 최종:ai · 갱신:2026-09-04
  - 짧은 제목: 토큰 게이트 하드월
  - 목적: 늘면 실패하던 게이트를 있으면 실패로 올려, 토큰 밖 값이 새로 들어오는 길을 완전히 닫는다
  - 이유: 승격 조건 「인식층이 다 열린 뒤의 래칫 0」이 네 번 미뤄진 끝에 KAN-077 로 처음 갖춰졌다
  - 목표: 위반·드리프트가 하나라도 있으면 CI 가 막고, 기준선 파일이 「지금 수」가 아니라 「0」을 뜻하게 된다
  - 메모: 승격은 네 번 미뤄졌다 — KAN-072·074·075·076 이 각각 래칫을 0 으로 만들었지만 그때마다 게이트가 못 보던 인식 갈래가 새로 열려 다시 비0 이 됐다(KAN-073 셋 → 0 이 778 로, KAN-075 넷째, KAN-076 다섯째, KAN-077 여섯째). **KAN-077 이 마지막 갈래를 열었고 지금 위반 0 · 드리프트 0 이다.** 진단 정본 design-concept/UI_CONSISTENCY_AUDIT.md §6 에 남은 13항(한 줄 200자 초과 선언)은 지금 src/** 에 0건이라 열 대상이 아니다. **왜 위험한가:** .github/workflows/main.yml 에서 게이트가 build 잡 안 astro build 앞에 돌고 deploy 가 needs: build 로 물려 있어, 하드월로 올린 뒤 위반이 하나라도 들어오면 **사이트 배포까지 멈춘다.** 그래서 인식층을 넓힌 커밋과 안 섞고 별건으로 뺐다(KAN-077 검토 5항, 유저 승인 2026-08-30). **정할 것 셋:** ① 「정당한 예외 10」과 「판정 불가 1130」은 하드월 대상이 아니다 — 기준선 파일에 그 둘을 계속 둘지, 예외만 남길지 ② 승격 뒤 기준선(scripts/tokens-baseline.json)의 역할이 「지금 수」에서 「0 이어야 한다」로 바뀌므로 --update-baseline 이 위반을 굳히는 문이 되면 안 된다 ③ selfTestFaults 래칫(KAN-076 S8)과 예외 표 자가검사는 그대로 둔다. **착수 첫 수는 그 시점의 래칫이 여전히 0 인지 확인하는 것이다** — 아니면 이 카드가 CI 를 빨갛게 만들면서 착수하게 되고 그동안 배포가 멈춘다.
  - 실행 문서: KANBAN.cards/KAN-079-1QVHJ2.md (9/9 · 최근 09-04)
  - 계획 리포트: KANBAN.reports/KAN-079-1QVHJ2.report.html (낡음)
  - 검토 문서: KANBAN.reviews/KAN-079-1QVHJ2.review.md (승인 0/6 · 검토 대기)
  - 원문:
    ```text
    세워
    ```
...
- `KAN-009` KAN-009 집필 ai의 퍼소나 개념을 정의하고 관리하려고 한다. 퍼소나는 고유의 voice와 캐릭터 이미지를 가진다. — 생성:유저 · 최종:유저 · 갱신:2026-07-23
  - 실행 문서: KANBAN.cards/KAN-009.md (0/1)
  - 원문:
    ````text
...
- `KAN-006` WebRTC 이해와 최적 활용 사례 — 생성:ai · 최종:ai · 갱신:2026-07-22
  ...
  - 실행 문서: KANBAN.cards/KAN-006.md (0/1)
...
- `KAN-050` 헤더 레벨로 포스트 페이지 우측에 네비게이션을 보여주고, 헤더를 클릭하면 바로 스크롤해서 이동할 수 있는 기능을 추가해라. — 생성:유저 · 최종:ai · 갱신:2026-07-29
  ...
  - 실행 문서: KANBAN.cards/KAN-050.md (0/1)
...
- `KAN-066-KK9Q50` 폭신 대담: 대화 원본 → 정형 구조화 저장 기능 (원문 보존 포함) — 생성:유저 · 최종:ai · 갱신:2026-08-18
  ...
  - 원문:
    ```text
    빵토교수님과의 대담 컨텐츠를 만들기 위해, 대화 원본을 제공하면, 최적 구조화 하여 저장하는 기능이 있어야 한다. 최적 구조화 에서는 데이터 구조, 파일 형식을 정형화 하고, 대화의 톤앤 매너, 비문, 맞춤법을 교정한다. 개선 방향성 수정에 대비해서 대화 원문 또한 보존해야 한다.
    ```
- `KAN-067-NH9SDQ` 폭신 대담에서 사용할 주제는 아래와 같다. 미리 주제에 대해 리서치를 하고, 대화를 대비한 준비를 해둬라. — 생성:유저 · 최종:ai · 갱신:2026-08-19
  ...
  - 원문:
    ```text
    폭신 대담에서 사용할 주제는 아래와 같다. 미리 주제에 대해 리서치를 하고, 대화를 대비한 준비를 해둬라.
    - 배치 프로세스로 처리되어 저장된 대규모 aggregated 데이터를 db에 저장하고, 있고, 프론트엔드에서 대규모 aggregated 데이터를 요청하는 시나리오가 있다고 하자. 프론트엔드는 모니터링, 인사이트 기능을 수행해서, 한 페이지에 여러 시각화 기능들이 존재한다. 따라서 단순 시나리오에서는 페이지를 호출할 때 페이지에 필요한 모든 데이터를 한번에 가져오게 되고, 반응성에 영향을 주게 된다. 이를 가장 효과적으로 해결할 수 있는 전략들이 무엇이 있는지 논의해 보자. 성능 측면에서의 해결방법과 ux 측면에서의 해결 방법 등, 여러 관점에서 문제를 바라보고 해결해 보자.
      - 내 의견1은, service worker, indexeddb를 활용하여 데이터를 프리패칭 하고, 캐시 하여, 이후에는 비어있거나 staled된 데이터만 받아오는 것이다. 이 때 비어있거나 staled data를 판단해서 패치하는 효과적인 전략이 있는지 고민해보고자 하낟.
      - 내 의견2는, http가 아니라 socket을 사용하고, 데이터를 압축해서 스트리밍하는 것이다. 이 또한 구체적인 방법과 최적화 관련 논의를 하고 싶다.
      - 내 의견3은, fallback skeleton과 loading ui를 적용하는 것이다.
      - 그 외에도 내가 생각하지 못한 방법이 있으면 알려달라.
    - 프론트로부터의 요청이 영속성 데이터를 불러오는 것부터 시작하면, 성능은 db query 성능에 지배 된다. 특정 자료구조나 알고리즘을 사용하는 것이 효과적임에도 불구하고, 일반적인 백엔드 로직에서는 개입의 여지가 없다. 이를 해결 하기 위해선 어떤 전략을 구사할 수 있을까. 알고리즘이나 자료구조를 이용하려면 일단, 데이터를 버퍼 위에 올려 놓는 수 밖에는 없는 것 같은데.
    ```
...
- `KAN-069-M724GH` UI 일관성을 전수 검토하여 진단 및 개선 방안에 대한 레포트를 제출해라. — 생성:유저 · 최종:ai · 갱신:2026-08-24
  ...
  - 원문:
    ...
    ```
- `KAN-070-DRF0QF` UI 토큰 게이트를 세운다 — verify-tokens 래칫 신설과 집필 규칙 개정 — 생성:ai · 최종:ai · 갱신:2026-08-26
  - 짧은 제목: 토큰 게이트 신설
  - 목적: 토큰 밖 값이 새로 들어오는 것을 CI 에서 그 자리에 막는 게이트를 세운다
  - 이유: 감사 판정기가 세션 스크래치패드에만 있어 지워지면 수치 재현이 불가하고, 게이트가 없으면 치환 성과를 매번 사람이 세야 한다
  ...
  - 원문:
    ...
    ```
- `KAN-072.3-SC3ADV` 축 재분류 — 간격 위반을 여백·치수·위치 셋으로 가른다 (KAN-070 S5 로 흡수) — 생성:ai · 최종:ai · 갱신:2026-08-27
  - 상위: `KAN-072-CPJCT1` (완료)
  - 짧은 제목: 축 재분류
  - 목적: DESIGN_CONCEPT 9절이 실제로 다스리는 범위만 남기고 나머지를 관할 밖으로 내린다
  - 이유: 감사의 간격 축 정의에 width·height·top·left 가 섞여 있는데 9절은 여백 규칙이지 치수나 좌표 규칙이 아니다
  - 목표: 치수·위치가 위반 목록에서 빠지고 감사 문서 3-5절 수치가 새 축 정의로 고쳐진다
  - 메모: 코드 변경 0 — 판정 규칙만 바꾼다. 위치 축 상위 파일이 전부 데코 실측 좌표(deco.astro 26 · PhotoFrame 23)라 스냅을 돌리면 조용히 깨진다. **KAN-070 S5 가 이미 다 했다(2026-08-27 확인)** — docrule.ts:56-62 가 26개 속성을 여백/치수/위치로 가르고 :243-252 가 뒤 둘을 판정 불가로 내린다. 감사 정본도 UI_CONSISTENCY_AUDIT.md:260 이 572 이고 :268-279 에 정정 블록이 붙었다. 확정치는 572 이고 옛 메모의 574 는 폐기된 수다(레포 전체에서 그 메모 한 곳에만 남아 있었다).
- `KAN-072.1-5XC1AT` 판단 없는 치환 — 드리프트 71 · 못 가르는 색 46 · fallback 23 — 생성:ai · 최종:ai · 갱신:2026-08-28
  - 상위: `KAN-072-CPJCT1` (완료)
  - 짧은 제목: 판단 없는 치환
  - 목적: 값이 이미 같거나 눈으로 못 가르는 자리를 토큰으로 되돌린다
  - 이유: 무엇으로 바꿀지 물을 것이 없는 자리라 가장 먼저 닫히고, 토큰을 고쳐도 이 자리들만 안 따라온다
  - 목표: 게이트의 드리프트 축과 fallback 축이 0 이 되고 화면은 안 바뀐다
  - 메모: 감사 P0-1~P0-4. rgba 전개 3건은 color-mix 로(DECO_KIT.md:32). motifs/** 는 제외한다 — SVG presentation attribute 라 var() 가 무효값이 되고 값 동일이 뜻 동일이 아니라 C4 로 넘긴다
- `KAN-072.2-BF75X5` 토큰 체계 보강 — 종이 흰색 · 선 굵기 2단 · radius · 상태색 · 간격/글자 스케일 토큰 — 생성:ai · 최종:ai · 갱신:2026-08-28
  - 상위: `KAN-072-CPJCT1` (완료)
  - 짧은 제목: 토큰 체계 보강
...
- `KAN-072.4-GJ9M4N` 글자 축 토큰화 — 문서 역할값 밖 166건을 --text-* 로 — 생성:ai · 최종:ai · 갱신:2026-08-28
  - 상위: `KAN-072-CPJCT1` (완료)
  - 짧은 제목: 글자 축 토큰화
  - 목적: 글자 크기 리터럴을 문서가 정한 역할값 토큰으로 바꾼다
  - 이유: 값 종류가 37종인데 문서가 정한 역할은 6개이고, 하한 12px 아래가 71건이다
  - 목표: 문서 역할값 밖 95건과 최소치 미만 71건이 0 이 된다
  - 메모: 발행물을 하나도 안 건드린다 — 166건 중 components/posts/** 가 0건이다(카탈로그 45·공용 66·데코 24·레이아웃 17·viz 11·전역 3). 그래서 간격 축보다 먼저 간다
- `KAN-072.5-WQV1KT` 간격 축 토큰화 — 스케일 밖 여백을 --space-* 로 (구획 순서로 위험을 뒤로) — 생성:ai · 최종:ai · 갱신:2026-08-28
  - 상위: `KAN-072-CPJCT1` (완료)
  - 짧은 제목: 간격 축 토큰화
  - 목적: 여백 리터럴을 4px 기준 스케일 토큰으로 바꾸고 스케일 자체를 흡수 가능하게 넓힌다
  - 이유: 가장 큰 단일 어긋남이고, 지배값이 전부 스케일 사이에 낀 값이라 스냅 방향이 곧 화면 변화다
  - 목표: 간격 축 위반이 0 이 되고 화면 변화가 최소로 억제된 것이 preview 로 확인된다
  - 메모: 9절에 16px 아래 2px 반 단을 넣어 330건(57%)을 화면 변화 0 으로 흡수한다. 동률이 56%라 최근접만으로는 273자리를 일제히 조인다. 구획 순서는 카탈로그·공용 → 데코·레이아웃 → posts(176건, C4 와 한 배치)
- `KAN-072.6-2YYK6V` 제2 팔레트 정리 — 글 안 시뮬 389 · 마스코트 28 · 카탈로그/데코/viz 103 — 생성:ai · 최종:ai · 갱신:2026-08-28
  - 상위: `KAN-072-CPJCT1` (완료)
  - 짧은 제목: 제2 팔레트 정리
  - 목적: 사이트 잉크와 계열이 다른 두 번째 팔레트를 걷어 한 벌로 만든다
  - 이유: 준수율 3.6% 구획이 본문 잉크를 코발트 남색이 아닌 따뜻한 먹색으로 쓰고 있어 같은 화면에 두 잉크가 선다
  - 목표: 구획 준수율이 회복되고, 해당 글을 preview 로 열어 시뮬이 정상 동작하는 것이 확인된다
  - 메모: 따뜻한 회색 4종은 흡수하고 상태 역할 5종은 C2 의 --state-* 로 간다. --sim-* 전용 층은 안 만든다(deco.css 1-33행의 조건 2를 못 넘는다). 매핑표는 코드모드 변환표이지 레포 파일이 아니다 — posts 자급식 원칙(REACT_SIM_GUIDE.md:10). 마스코트 28은 치환이 아니라 팔레트를 DESIGN_CONCEPT 7절에 적는 일이다
- `KAN-072.7-D6DCXJ` 게이트 인식층 — 토큰 이름 규칙을 하드코딩에서 데이터로 뺀다 — 생성:ai · 최종:ai · 갱신:2026-08-28
  - 상위: `KAN-072-CPJCT1` (완료)
  - 짧은 제목: 게이트 인식층
  - 목적: 신설 토큰을 게이트가 자기 축으로 읽게 한다
  - 이유: 지금은 토큰 이름이 코드에 박혀 있어 새 토큰 5종이 게이트에 안 잡히고, 그러면 토큰을 세워도 위반 수가 안 준다
  - 목표: tokens.css 에 토큰을 더하면 게이트가 그 축으로 자동 판정한다 — 코드를 다시 안 고친다
  - 메모: KAN-070 전략이 전제했으나 구현 안 된 전환. color.ts:54 COLOR_TOKENS 18개 고정 · :66 --stroke 정확 일치 · docrule.ts:66,79 SPACING_SCALE/FONT_ROLES 상수. radius 만 color.ts:64 includes 라 이미 자동이다. 자가검사가 reason 문자열을 매칭하므로(selftest.ts:69) 판정 문구를 바꾸면 픽스처도 함께 간다. 이 카드만 scripts/lib/tokens/** scope 를 진다 — 유저 선택 2026-08-27
- `KAN-072-CPJCT1` UI 토큰 정합화 — 게이트가 무는 1,786건을 구획 단위로 0 으로 줄인다 — 생성:ai · 최종:ai · 갱신:2026-08-28
  - 짧은 제목: 토큰 정합화
  - 목적: src/** 의 토큰 밖 값과 문서 규칙 밖 값을 토큰으로 되돌린다
  - 이유: 준수율이 44.3% 이고 무너진 구획 둘(글 안 시뮬 3.6% · 마스코트 8.1%)이 발행물 안에 있다
  ...
  - 검토 문서: KANBAN.reviews/KAN-072-CPJCT1.review.md (승인 8/8 · 추가 의견 총 2 · 승인)
  - 원문:
    ```text
    위 artifact를 보고 전략과 실행 계획을 수립해라
    ```
- `KAN-064` 집필 엔진 교체(authoring-kit)에 따른 기존 글 재집필 계획 수립 — 생성:유저 · 최종:ai · 갱신:2026-08-28
...
- `KAN-071-6361MY` 선언 층을 줄인다 — bbangtoTonyStyleGuide export 를 걷고 foundation 만 남긴다 — 생성:ai · 최종:ai · 갱신:2026-08-28
  ...
  - 실행 문서: KANBAN.cards/KAN-071-6361MY.md (3/3 · 최근 08-28)
  - 검토 문서: KANBAN.reviews/KAN-071-6361MY.review.md (승인 5/5 · 추가 의견 총 3 · 승인)
  - 원문:
    ```text
    위 artifact를 보고 전략과 실행 계획을 수립해라
    ```
- `KAN-073-CVD8X7` 게이트 인식층 확장 — 추출층이 못 보는 두 갈래를 판정에 넣는다 — 생성:ai · 최종:ai · 갱신:2026-08-28
  ...
  - 검토 문서: KANBAN.reviews/KAN-073-CVD8X7.review.md (승인 6/6 · 승인)
  - 원문:
    ```text
    ...
    ```
- `KAN-074-MQ9KRB` 토큰 게이트 부채 상환 — 인식층 확장으로 드러난 778건을 옮긴다 — 생성:ai · 최종:ai · 갱신:2026-08-29
  - 짧은 제목: 게이트 부채 상환
  - 목적: KAN-073 이 판정에 올린 위반 170·드리프트 608 을 토큰으로 옮겨 래칫을 0 으로 되돌린다
  - 이유: 래칫이 0 이어야 하드월로 승격하는데 지금 778 에서 버티고 있다
  - 목표: bun run tokens:verify 의 래칫 대상이 0 이 되고 화면 변화가 검증된 상태로 남는다
...
- `KAN-077-RA2SYG` 게이트 회피 경로를 막는다 — 상수로 끌어올린 리터럴 15건을 판정에 넣는다 — 생성:ai · 최종:ai · 갱신:2026-08-31
  - 짧은 제목: 상수 승격 리터럴
  - 목적: const 로 빼놓은 시각 값이 게이트 시야를 벗어나는 구멍을 막는다
  - 이유: 판정 대기 리터럴을 상수로 올리기만 하면 게이트를 피할 수 있어, 「0」이 닫힌 것과 숨은 것을 섞는다
  - 목표: const 에 든 시각 값이 판정에 들어오고, 상수로 올려 게이트를 피하는 길이 닫힌다
  - 메모: KAN-073 검토 항목 5 승인(2026-08-28). 실측 15건이고 3색이 6파일에 복제돼 있다(SAND #e8c97a · TEAL_TINT #e5f0ed · MUTED #b8b0a0, 주석에 최근접 토큰과 색차까지 적혀 있는데 근거를 걸 자리가 없어 게이트가 못 본다). 다른 갈래와 기법이 다르다 — 정규식으로는 그 상수가 시각 값인지 알 수 없고 사용처를 따라가야 한다. 실측 전례: KANBAN.cards/KAN-072-CPJCT1.md:223 에서 상수로 올렸다가 시야를 벗어나 6건을 원래 인라인 자리로 되돌린 기록이 있다
  - 실행 문서: KANBAN.cards/KAN-077-RA2SYG.md (10/10 · 최근 08-30)
  - 계획 리포트: KANBAN.reports/KAN-077-RA2SYG.report.html (낡음)
  - 검토 문서: KANBAN.reviews/KAN-077-RA2SYG.review.md (승인 5/5 · 추가 의견 총 4 · 승인)
- `KAN-078-D44PD1` render:compare 가 대조를 안 하고도 「전후 동일」이라고 답한다 — 거짓 통과를 막는다 — 생성:ai · 최종:ai · 갱신:2026-08-31
  - 짧은 제목: compare-render 거짓 통과
  - 목적: 화면 무변경을 증명하는 도구가 대조를 건너뛰고도 통과로 끝나는 자리를 막는다
  - 이유: KAN-077 에서 실제로 밟았다 — 「전후 동일」을 받은 뒤 옵션만 빼고 다시 돌리니 28건이 나왔다
  - 목표: 자가검사만 돌린 실행이 「전후 동일」로 끝나지 않고, 같은 거짓 통과가 다시 나면 검사가 문다
  - 메모: scripts/compare-render.ts:667 에서 SELF_TEST 갈래와 실제 전후 대조가 if/else 로 갈리는데 파일 끝의 console.log("✓ 전후 동일.") 이 두 갈래 공통이라, 자가검사만 돌린 실행이 **대조를 한 번도 안 하고** 통과로 끝난다. 실측(KAN-077 S8): S7 직후 --self-test 로 돌려 「✓ 전후 동일」을 받았고 옵션만 빼고 다시 돌리니 background-color 28건 차이가 나왔다 — 그 사이 코드는 한 글자도 안 바뀌었다. 이 도구는 「값을 바꿨는데 화면이 안 바뀌었음」을 증명하려고 만든 것이라 그 증명이 거짓으로 통과하는 것은 도구 자신이 막으려던 실패 모드다. 고칠 것은 한 줄이지만 **고치는 것으로 끝내지 않는다** — 그 거짓 통과를 재현하는 자가검사를 함께 둔다(KAN-076 S8 이 selfTestFaults 로 같은 종류의 자기 축소를 막은 선례). 갈래 후보 둘: ①자가검사 뒤에도 실제 대조를 이어서 돈다 ②자가검사만 돌았으면 마지막 줄을 「자가검사 통과(대조 안 함)」로 가른다. KAN-077 은 이 파일이 scope 밖이라 안 고치고 넘겼다(검토 4항, 유저 판정 「새카드」 2026-08-30).
  - 실행 문서: KANBAN.cards/KAN-078-D44PD1.md (3/3 · 최근 08-31)
  - 계획 리포트: KANBAN.reports/KAN-078-D44PD1.report.html (낡음)
  - 검토 문서: KANBAN.reviews/KAN-078-D44PD1.review.md (승인 5/5 · 승인)
  - 원문:
    ```text
    새카드
    ```
- `KAN-080-WTQ7AV` 타이포그래피가 하나의 디자인 스타일 안에서 일관성 없이 정의되어 있다. 하나의 디자인 스타일을 준수하는 타이포그래피 조합을 조사하여 채택해라. — 생성:유저 · 최종:ai · 갱신:2026-09-03
  - 짧은 제목: 타이포그래피 체계 채택
  - 목적: 폰트·크기만 있는 타이포 토큰에 굵기·행간·자간 역할을 세우고, 한 디자인 스타일을 따르는 조합을 조사해 채택한다
