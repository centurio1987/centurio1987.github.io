...
- `KAN-072.3-SC3ADV` 축 재분류 — 간격 위반 821건을 간격·치수·위치 셋으로 가른다 (판정만) — 생성:ai · 최종:ai · 갱신:2026-08-25
  - 상위: `KAN-072-CPJCT1` (할 일)
  - 짧은 제목: 축 재분류
  ...
  - 메모: 코드 변경 0 — 판정 규칙만 바꾼다. 위치 축 상위 파일이 전부 데코 실측 좌표(deco.astro 26 · PhotoFrame 23)라 스냅을 돌리면 조용히 깨진다. 정확한 수는 판정기 이식 뒤 확정(검토자 572·내 재계산 574로 갈린다)
